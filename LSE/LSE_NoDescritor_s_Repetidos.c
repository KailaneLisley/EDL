// LSE com Nó Descritor - NÃO PERMITE VALORES REPETIDOS
...
int existeValor(Descritor* lista, int valor);           //Verifica se valor já existe na lista
...
void removerFim (Descritor* lista);                   //Procedimento para Remover Último Elemento da Lista
...
// F U N Ç Ã O   A U X I L I A R   P A R A   V E R I F I C A R   V A L O R   R E P E T I D O

int existeValor(Descritor* lista, int valor){           //Verifica se valor já está na lista
    NoLSE* aux = lista->prim;                           //Começa do primeiro nó
    while(aux != NULL){                                 //Percorre a lista
        if(aux->info == valor){                         //Se encontrar o valor
            return 1;                                   //Retorna verdadeiro (valor já existe)
        }
        aux = aux->prox;                                //Avança para o próximo nó
    }
    return 0;                                           //Se não encontrar, retorna falso
}

void InserirInicio (Descritor* d, int valor){           // I N S E R I R   V A L O R   N O   I N I C I O   D A   L I S T A
    if (existeValor(d, valor)) {                        //Verifica se valor já existe
        printf("Valor %d já está na lista. Inserção ignorada.\n", valor);
        return;                                         //Não insere se for repetido
    }
