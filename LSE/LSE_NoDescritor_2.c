#include <stdio.h>
#include <stdlib.h>

typedef struct noLSE {
    int info;
    struct noLSE* prox;
} NoLSE;

typedef struct descritor {
    NoLSE* prim;
    NoLSE* ult;
    int qtd;
} Descritor;

Descritor* criarLista();
NoLSE* criarNo(int valor);
void exibirOpcao();
void inserirInicio(Descritor* d, int valor);
void inserirFinal(Descritor* d, int valor);
void exibirLista(Descritor* d);
void removerInicio(Descritor* d);
void removerFinal(Descritor* d);
void removerValor(Descritor* d, int valor);
NoLSE* buscarValor(Descritor* d, int valor);
void liberarLista(Descritor* d);

int main() {
    Descritor* lista = criarLista();
    int opcao, valor;
    do {
        exibirOpcao();
        scanf("%d", &opcao);
        
        switch(opcao) {
            case 1:
                printf("Digite o valor: ");
                scanf("%d", &valor);
                inserirInicio(lista, valor);
                break;
            case 2:
                printf("Digite o valor: ");
                scanf("%d", &valor);
                inserirFinal(lista, valor);
                break;
            case 3:
                removerInicio(lista);
                break;
            case 4:
                removerFinal(lista);
                break;
            case 5:
                printf("Digite o valor a ser removido: ");
                scanf("%d", &valor);
                removerValor(lista, valor);
                break;
            case 6:
                printf("SUA LISTA\n");
                exibirLista(lista);
                break;
            case 7:
                printf("Digite o valor a ser procurado: ");
                scanf("%d", &valor);
                if (buscarValor(lista, valor) != NULL)
                    printf("Valor encontrado na lista.\n");
                else
                    printf("Valor nao encontrado.\n");
                break;
            case 0:
                printf("Encerrando programa...\n");
                liberarLista(lista);
                break;
            default:
                printf("Opção inválida!\n");
        }
    } while(opcao != 0);
    return 0;
}

Descritor* criarLista() {
    Descritor* aux = (Descritor*) malloc(sizeof(Descritor));
    aux->prim = aux->ult = NULL;
    aux->qtd = 0;
    return aux;
}

NoLSE* criarNo(int valor) {
    NoLSE* aux = (NoLSE*) malloc(sizeof(NoLSE));
    aux->info = valor;
    aux->prox = NULL;
    return aux;
}

void exibirOpcao() {
    printf("\n1 - Inserir no início\n");
    printf("2 - Inserir no final\n");
    printf("3 - Remover o primeiro elemento\n");
    printf("4 - Remover o último elemento\n");
    printf("5 - Remover um valor específico\n");
    printf("6 - Exibir a lista\n");
    printf("7 - Procurar valor na lista\n");
    printf("0 - Sair\n");
    printf("Escolha uma opção: ");
}

void inserirInicio(Descritor* d, int valor) {
    if (buscarValor(d, valor) != NULL) {
        printf("Valor já existe na lista!\n");
        return;
    }
    NoLSE* novo = criarNo(valor);
    novo->prox = d->prim;
    d->prim = novo;
    if (d->qtd == 0) d->ult = novo;
    d->qtd++;
}

void inserirFinal(Descritor* d, int valor) {
    if (buscarValor(d, valor) != NULL) {
        printf("Valor já existe na lista!\n");
        return;
    }
    NoLSE* novo = criarNo(valor);
    if (d->qtd == 0) {
        d->prim = d->ult = novo;
    } else {
        d->ult->prox = novo;
        d->ult = novo;
    }
    d->qtd++;
}

void exibirLista(Descritor* d) {
    if (d->qtd == 0) {
        printf("Lista vazia!\n");
        return;
    }
    NoLSE* aux = d->prim;
    printf("Lista: ");
    while(aux) {
        printf("%d ", aux->info);
        aux = aux->prox;
    }
    printf("\n");
}

void removerInicio(Descritor* d) {
    if (d->qtd == 0) return;
    NoLSE* aux = d->prim;
    d->prim = aux->prox;
    free(aux);
    d->qtd--;
    if (d->qtd == 0) d->ult = NULL;
}

void removerFinal(Descritor* d) {
    if (d->qtd == 0) return;
    if (d->qtd == 1) {
        free(d->prim);
        d->prim = d->ult = NULL;
    } else {
        NoLSE* aux = d->prim;
        while (aux->prox != d->ult) aux = aux->prox;
        free(d->ult);
        d->ult = aux;
        d->ult->prox = NULL;
    }
    d->qtd--;
}

void removerValor(Descritor* d, int valor) {
    NoLSE* atual = d->prim, *ant = NULL;
    while (atual && atual->info != valor) {
        ant = atual;
        atual = atual->prox;
    }
    if (!atual) {
        printf("Valor não encontrado!\n");
        return;
    }
    if (ant) ant->prox = atual->prox;
    else d->prim = atual->prox;
    if (atual == d->ult) d->ult = ant;
    free(atual);
    d->qtd--;
}

NoLSE* buscarValor(Descritor* d, int valor) {
    NoLSE* aux = d->prim;
    while (aux) {
        if (aux->info == valor) return aux;
        aux = aux->prox;
    }
    return NULL;
}

void liberarLista(Descritor* d) {
    NoLSE* aux = d->prim;
    while (aux) {
        NoLSE* temp = aux;
        aux = aux->prox;
        free(temp);
    }
    free(d);
}